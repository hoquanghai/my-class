...
- Route groups: `(auth)` teacher login/signup/reset; `app/` teacher shell (client layout gated by `useMe()`, nav Classes / Questions / Quizzes); `(student)` `/join`, `/join/[code]`, `/s` (mobile-first, no chrome; `/student/me` returns the currently open session and run); `(marketing)`. `/present/[sessionId]` is not built yet.
