...
- Images inside question content are stored as the reference `media:<key>` (`packages/shared/src/media.ts`), never as a provider URL: question text is snapshotted into runs and shared community sets, so a hard-coded URL would break every old question if the storage provider, CDN or domain changes. The browser resolves a reference through `GET /api/media/f/<key>`, which 302-redirects to the Spaces URL (or serves bytes with the `memory` driver); the single resolution point on the web is the `img` renderer in `components/markdown-latex.tsx`. Absolute URLs already in old content still render untouched. `Teacher.avatarUrl` is the exception: it holds an absolute URL because OAuth providers supply one.
...
`packages/shared/src/plans.ts` is the single source for plan ids (`free | gold | platinum`), prices (Gold 100k/month, Platinum 200k/month; yearly = 70% of 12 months rounded to 10k → 840k / 1,680k, user decision 2026-09-11), `PLAN_LIMITS` (display only for now: enforcement still reads the `free.*` flags) and `transferMemo()`. `components/pricing/pricing-table.tsx` renders the three cards with a monthly/yearly toggle and a collapsible comparison table whose rows live in `vi.json` (`Pricing.compare.rows`); it is used on the landing section `#mien-phi` (`variant="marketing"`, CTAs to `/signup`) and on `/app/upgrade` (`variant="app"`, opens `OrderDialog`). Payment is manual bank transfer: `OrderDialog` reads `NEXT_PUBLIC_BILLING_BANK/ACCOUNT/OWNER/ZALO` and shows the memo `LOPHOC GOLD 12T <email>`; with empty env it shows a "contact support" notice. Features not built yet are labelled "(sắp có)" in the plan lists.

Billing (manual bank transfer, user decision 2026-09-11): `OrderDialog` shows the VietQR image for the chosen plan/cycle (`apps/web/public/img/qr/<plan>-<cycle>.webp`, cropped from the TPBank screenshots kept untracked in `/bank`; account constants in `BANK_ACCOUNT`), the memo `transferMemo()`, then "Tôi đã chuyển khoản" → `POST /billing/requests` creates a `PaymentRequest` (`pending`; only one pending per teacher, else 409 `PAYMENT_PENDING`) and emails `BILLING_ADMIN_EMAIL` with a review link `GET /api/billing/review/:id?token=` (HMAC of the id with `JWT_SECRET`, `@Public`). That page's forms `POST …/confirm` or `…/reject` set the status, on confirm write `Teacher.plan` + `planExpiresAt` via `extendExpiry()` (extends the current term when the plan is the same) and email the teacher. `TeacherDto.plan` is the _effective_ plan (`effectivePlan()`: expired paid plans read as `free`) plus `planExpiresAt`; `GET /billing/me` returns plan, pending request and history, shown on `/app/upgrade` and in the account menu. Plan limits are still not enforced per plan.

### Authentication

- Teacher: argon2 password, Google OAuth, or Facebook Login. Both providers implement `OAuthProviderService` (`modules/auth/oauth-profile.ts`) and share `AuthService.loginWithOAuth(provider, profile)` plus the generic `oauthStart`/`oauthCallback` handlers in `auth.controller.ts`; a provider whose env vars are empty is hidden by `GET /auth/providers` and its start route returns 503. Callback failures redirect to `/login?error=<provider>`. Access JWT lasts 15 min in httpOnly cookie `lh_at`; refresh token lasts 30 days, is rotated and stored hashed, in cookie `lh_rt` scoped to path `/api/auth`. A non-httpOnly hint cookie `lh_session` is read by `apps/web/src/proxy.ts` to redirect unauthenticated `/app/*` to `/login`; the real gate is `useMe()` in `app/app/layout.tsx`. Cookie helpers are in `modules/auth/cookies.ts`. Return-to after auth: `proxy.ts` and the app layout send `/login?next=<path>`; the login and signup pages honor it (`lib/next-path.ts` `safeNext`, internal paths only; signup maps `?plan=` to `/app/upgrade`), the social buttons pass it as `GET /auth/google?next=`, and the API keeps it in the `lh_oauth_next` cookie (validated by `safeNextPath`) so the OAuth callback redirects to `${APP_URL}${next}` instead of always `/app/classes`.
