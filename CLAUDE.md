...
- AI import accepts images (≤10), or one document: PDF (≤20 pages), Word `.docx` (text via mammoth; MathType OLE objects are counted and reported in `warnings` because their formulas are lost), or `.tex`/`.txt`/`.md` (sent as a `text` page; quota = 3,000 chars per page). Catalog of accepted types: `packages/shared/src/questions/ai-files.ts`; `ExtractionPage` has kind `text` and every provider sends it as a text block.
