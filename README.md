...
- Trích xuất AI: `AI_PROVIDER=cascade` dùng Gemini 3.7 Flash đọc cả đề, bộ chấm chất lượng (không tốn token) tìm trang có câu lỗi rồi chỉ gửi lại các trang đó cho Claude Sonnet 5; `AI_ESCALATION_MAX_SHARE` giới hạn tỷ lệ trang leo thang. Kết quả job lưu `meta` (model, token, USD ước tính, trang đã leo thang).
